...
class SimpleTrainer(TrainerBase):
    ...
    def after_train(self,idx=-1,epoch=0,is_fed=False):
        ...
        for param_group in self.optim.param_groups:
            # 对于每个参数组，我们获取首个参数的状态字典
            for param in param_group['params']:
                param_state = self.optim.state[param]
                # 检查是否存在动量缓冲区
        ...
        if do_test:
            ...
            # self.test(idx=idx,current_epoch=epoch,is_global=is_fed,global_test=False)
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self, idx=-1, global_epoch=-1, **kwargs):
        ...
        for self.batch_idx, batch in enumerate(loader):
            ...
            data_time.update(time.time() - end)
            loss_summary = self.forward_backward(self.batch_idx, batch, idx, **kwargs)
            batch_time.update(time.time() - end)
            losses.update(loss_summary)
